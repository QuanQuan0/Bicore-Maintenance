...
// 通用的函数用于保存二维向量到CSV文件
void save_vector_to_csv(const std::vector<std::vector<int>>& vec, const std::string& filename) {
    std::ofstream file(filename);
    for (const auto& row : vec) {
        for (size_t j = 0; j < row.size(); ++j) {
            file << row[j];
            if (j != row.size() - 1) {
                file << ",";  // 在每个值后添加逗号，除最后一个
            }
        }
        file << "\n";
    }
...
int main(int argc, char **argv)
{
    ...
    {
        vector<double> time_vector, bfstime_vector;
        ...
        while ((r = fscanf(edgenum, "%d %d", &u, &v)) != EOF)
        {
            ...
            insert_num++;
            auto time = Dyn_rebuild::update_skyline_index_swap_with_bfs(g, u, v, skyline_pair);
            ...
            cout << i << "th: " << " " << time << " " << mean <<  endl;
            // if (i == 10) break;
        }
        fclose(edgenum);
        double pair_num = skyline_pair / insert_num;
        double sum = std::accumulate(std::begin(time_vector), std::end(time_vector), 0.0);
        double mean = sum / time_vector.size();

        // save_vector_to_csv(g.left_index, "/home/quan/test/core_maintenanceV2/reproduce/output/left1.csv");
        // save_vector_to_csv(g.right_index, "/home/quan/test/core_maintenanceV2/reproduce/output/right1.csv");

        cout << "Multi-Edges-Insert running average time: " << mean << " " << pair_num << " " << delta << endl;
    }
