...
namespace Dyn_rebuild
{
	...
	void casacade_insert_candidate_nodes_skyline(BiGraph& g, vid_t node, int alpha, int beta, bool left_side,
		unordered_set<vid_t>& visited_nodes_left, unordered_set<vid_t>& visited_nodes_right,
		unordered_set<vid_t>& candidate_nodes_left, unordered_set<vid_t>& candidate_nodes_right,
		unordered_map<vid_t, traversal_info_removal>& visited_nodes_2_info_left, unordered_map<vid_t, traversal_info_removal>& visited_nodes_2_info_right){
		if (left_side){
			...
			for (auto v : visited_nodes_2_info_left[node].neighbors_not_in_candidates) {
				if (candidate_nodes_right.find(v) == candidate_nodes_right.end()) {
					visited_nodes_2_info_right[v].current_supports--;
					if (visited_nodes_2_info_right[v].current_supports < beta) {
						casacade_insert_candidate_nodes_skyline(g, v, alpha, beta, !left_side,
							visited_nodes_left, visited_nodes_right, candidate_nodes_left, candidate_nodes_right,
							visited_nodes_2_info_left, visited_nodes_2_info_right);
					}
				}
			...
			for (auto v : visited_nodes_2_info_left[node].nodes_to_be_visited) {
				if (visited_nodes_right.find(v) == visited_nodes_right.end()) {
                    dynamic_ergodic_change_by_satisfy_nbr_number_delete(g, alpha, beta,
                            v, !left_side, visited_nodes_left, visited_nodes_right, candidate_nodes_left, candidate_nodes_right,
                            visited_nodes_2_info_left, visited_nodes_2_info_right);
				}
			}
		}
			...
			for (auto u : visited_nodes_2_info_right[node].neighbors_not_in_candidates) {
				if (candidate_nodes_left.find(u) == candidate_nodes_left.end()) {
					visited_nodes_2_info_left[u].current_supports--;
					if (visited_nodes_2_info_left[u].current_supports < alpha) {
						casacade_insert_candidate_nodes_skyline(g, u, alpha, beta, !left_side,
							visited_nodes_left, visited_nodes_right, candidate_nodes_left, candidate_nodes_right,
							visited_nodes_2_info_left, visited_nodes_2_info_right);
					}
				}
			...
			for (auto u : visited_nodes_2_info_right[node].nodes_to_be_visited) {
				if (visited_nodes_left.find(u) == visited_nodes_left.end()) {
                    dynamic_ergodic_change_by_satisfy_nbr_number_delete(g, alpha, beta,
                            u, !left_side, visited_nodes_left, visited_nodes_right, candidate_nodes_left, candidate_nodes_right,
                            visited_nodes_2_info_left, visited_nodes_2_info_right);
				}
			}
		}
    ...
    void dynamic_ergodic_change_by_satisfy_nbr_number_delete(BiGraph& g, int alpha, int beta, 
        vid_t node, bool left_side, unordered_set<vid_t>& visited_nodes_left, unordered_set<vid_t>& visited_nodes_right,
		unordered_set<vid_t>& candidate_nodes_left, unordered_set<vid_t>& candidate_nodes_right,
		unordered_map<vid_t, traversal_info_removal>& visited_nodes_2_info_left, unordered_map<vid_t, traversal_info_removal>& visited_nodes_2_info_right){
		if(left_side){
            visited_nodes_left.insert(node);
			...
			if (visited_nodes_2_info_left[node].current_supports < alpha) {
				casacade_insert_candidate_nodes_skyline(g, node, alpha, beta, left_side,
					visited_nodes_left, visited_nodes_right, candidate_nodes_left, candidate_nodes_right,
					visited_nodes_2_info_left, visited_nodes_2_info_right);
				for (auto v : visited_nodes_2_info_left[node].nodes_to_be_visited) {
					if (visited_nodes_right.find(v) == visited_nodes_right.end()) {
						dynamic_ergodic_change_by_satisfy_nbr_number_delete(g, alpha, beta, v, !left_side, visited_nodes_left, visited_nodes_right,
			...
			if (visited_nodes_2_info_right[node].current_supports < beta) {
				...
				for (auto u : visited_nodes_2_info_right[node].nodes_to_be_visited) {
					if (visited_nodes_left.find(u) == visited_nodes_left.end()) {
						dynamic_ergodic_change_by_satisfy_nbr_number_delete(g, alpha, beta, u, !left_side, visited_nodes_left, visited_nodes_right,
                                                                candidate_nodes_left, candidate_nodes_right, visited_nodes_2_info_left, visited_nodes_2_info_right);
					}
				}
			}
	...
	void rebuild_skyline(BiGraph& g, std::vector<skyline_block*>& skyline_index_u, std::vector<skyline_block*>& skyline_index_u_reverse, std::vector<skyline_block*>& skyline_index_v, std::vector<skyline_block*>& skyline_index_v_reverse){
		//for (int i = 1; i < g.left_index_delete.size(); i++) {
		...
		// for (int i = 1; i < g.right_index_delete.size(); i++) {
		...
		skyline_index_u_reverse.clear(); skyline_index_v_reverse.clear(); 
		...
		g.left_index_old = g.left_index;
		g.right_index_old = g.right_index;

		g.left_index_delete = g.left_index;
		g.right_index_delete = g.right_index;
	}

	//void rebuild_skyline(BiGraph& g, std::vector<skyline_block*>& skyline_index_u, std::vector<skyline_block*>& skyline_index_u_reverse, 
	...
	double update_skyline_index_swap_with_bfs_delete(BiGraph& g, std::vector<skyline_block*>& skyline_index_u, std::vector<skyline_block*>& skyline_index_v,
        ...
        if(pair_to_be_visit.size() == 0){
            ...
            if(pair_to_be_visit.count(g.degree_v1[u] + 1) == 0){
				...
				pair_to_be_visit[g.degree_v1[u] + 1] = g.left_index[u][g.degree_v1[u] + 1];
            }
        }
        if(pair_to_be_visit_v.size() == 0){
            pair_to_be_visit_v[skyline_index_v_reverse[v] -> mapset[g.degree_v2[v] + 1]] = g.degree_v2[v] + 1;
        }
        else{
            if(pair_to_be_visit_v.count(g.degree_v2[v] + 1) == 0){
				...
				pair_to_be_visit_v[g.right_index[v][g.degree_v2[v] + 1]] = g.degree_v2[v] + 1;
            }
        }
        ...
        for (auto iter = pair_to_be_visit.rbegin(); iter != pair_to_be_visit.rend(); iter++){
			...
			for (auto w : candidate_nodes_left){
				...
				if (w == u){
					...
					while (g.left_index_change[w][i] < beta_2 && i < g.left_index[w].size()){
						if(beta_2 < g.left_index[w][i]) {
							g.left_index_change[w][i] = beta_2;
							if(g.left_index_delete[w][i] == 0) g.left_index_old[w][i] = beta_2;
						}
						i++;
					}
					if(g.left_index_delete[w][i] == 0 && g.left_index_old[w][i] == g.left_index[w][i]) g.left_index_old[w][i] = g.left_index_change[w][i];
				}
				else{
					int i = current_alpha;
					while (i < g.left_index[w].size() && g.left_index_change[w][i] < current_beta - 1){
						if(current_beta - 1 < g.left_index[w][i]) {
							g.left_index_change[w][i] = current_beta - 1;
							if(g.left_index_delete[w][i] == 0) g.left_index_old[w][i] = current_beta - 1;
						}
						i++;
					}
					if(g.left_index_delete[w][i] == 0 && g.left_index_old[w][i] == g.left_index[w][i]) g.left_index_old[w][i] = g.left_index_change[w][i];
				...
				if (g.left_index_change[w][current_alpha - 1] < beta_){
					if(beta_ < g.left_index[w][current_alpha - 1]) {
						g.left_index_change[w][current_alpha - 1] = beta_;
						if(g.left_index_delete[w][current_alpha - 1] == 0) g.left_index_old[w][current_alpha - 1] = beta_;
					}
				}
				else{
					if(g.left_index_delete[w][current_alpha - 1] == 0 && g.left_index_old[w][current_alpha - 1] == g.left_index[w][current_alpha - 1])
					g.left_index_old[w][current_alpha - 1] = g.left_index_change[w][current_alpha - 1];
				} 
			}
			for (auto w : candidate_nodes_right) {
				...
				if (g.right_index_change[w][current_beta - 1] < current_alpha){
					if(current_alpha < g.right_index[w][current_beta - 1]) {
						g.right_index_change[w][current_beta - 1] = current_alpha;
						if(g.right_index_delete[w][current_beta - 1] == 0) g.right_index_old[w][current_beta - 1] = current_alpha;
					}
				}
				else{
					if(g.right_index_delete[w][current_beta - 1] == 0 && g.right_index_old[w][current_beta - 1] == g.right_index[w][current_beta - 1])
					g.right_index_old[w][current_beta - 1] = g.right_index_change[w][current_beta - 1];
				}
				if (w == v){
					int alpha_2 = 0;
					...
					while (g.right_index_change[w][i] < alpha_2 && i < g.right_index[w].size()){
						if(alpha_2 < g.right_index[w][i]) {
							g.right_index_change[w][i] = alpha_2;
							if(g.right_index_delete[w][i] == 0) g.right_index_old[w][i] = alpha_2;
						}
						i++;
					}
					if(g.right_index_delete[w][i] == 0 && g.right_index_old[w][i] == g.right_index[w][i])
					g.right_index_old[w][i] = g.right_index_change[w][i];
				}
				else{
					for (int i = current_beta; i <= beta_; i++){ 
						if (g.right_index_change[w][i] < current_alpha - 1 && current_alpha - 1 < g.right_index[w][i]) {
							g.right_index_change[w][i] = current_alpha - 1;
							if(g.right_index_delete[w][i] == 0) g.right_index_old[w][i] = current_alpha - 1; 
						}
						else{
							if(g.right_index_delete[w][i] == 0 && g.right_index_old[w][i] == g.right_index[w][i])
							g.right_index_old[w][i] = g.right_index_change[w][i];
        ...
        for (auto iter = pair_to_be_visit_v.begin(); iter != pair_to_be_visit_v.end(); iter++){
			...
			for (auto w : candidate_nodes_right){
				...
				if (w == v){
					...
					if (g.right_index_delete[w][i] == 0 && g.right_index_old[w][i] == g.right_index[w][i]) g.right_index_old[w][i] = g.right_index_change[w][i];
				}
				else{
					int i = current_beta;
					while (g.right_index_change[w][i] < current_alpha - 1 && i < g.right_index[w].size()){
						if(current_alpha - 1 < g.right_index[w][i]) {
							g.right_index_change[w][i] = current_alpha - 1;
							if(g.right_index_delete[w][i] == 0) g.right_index_old[w][i] = current_alpha - 1;
						}
						i++;
				...
				if (g.right_index_change[w][current_beta - 1] < alpha_){
					if(alpha_ < g.right_index[w][current_beta - 1]){
						g.right_index_change[w][current_beta - 1] = alpha_;
						if(g.right_index_delete[w][current_beta - 1] == 0) g.right_index_old[w][current_beta - 1] = alpha_;
					}
				}
				else{
					if(g.right_index_delete[w][current_beta - 1] == 0 && g.right_index_old[w][current_beta - 1] == g.right_index[w][current_beta - 1])
			...
			for (auto w : candidate_nodes_left) {
				...
				if (g.left_index_change[w][current_alpha - 1] < current_beta){
					if(current_beta < g.left_index[w][current_alpha - 1]) {
						g.left_index_change[w][current_alpha - 1] = current_beta;
						if(g.left_index_delete[w][current_alpha - 1] == 0) g.left_index_old[w][current_alpha - 1] = current_beta;
					}
				}
				else{
					if(g.left_index_delete[w][current_alpha - 1] == 0 && g.left_index_old[w][current_alpha - 1] == g.left_index[w][current_alpha - 1])
				...
				for (int i = current_alpha; i <= alpha_; i++){ 
					if (g.left_index_change[w][i] < current_beta - 1 && current_beta - 1 < g.left_index[w][i]) {
						g.left_index_change[w][i] = current_beta - 1; 
						if(g.left_index_delete[w][i] == 0) g.left_index_old[w][i] = current_beta - 1; 
					}
					else{
						if(g.left_index_delete[w][i] == 0 && g.left_index_old[w][i] == g.left_index[w][i])
						g.left_index_old[w][i] = g.left_index_change[w][i];
					}
				}
			}
        }
		swap(g.left_index_old, g.left_index);
		swap(g.right_index_old, g.right_index);
