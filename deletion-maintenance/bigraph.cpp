...
void build_skyline_index(BiGraph&g, vector<vector<bicore_index_block*>>& bicore_index_u, vector<vector<bicore_index_block*>>& bicore_index_v, vector<skyline_block*>& skyline_index_u, vector<skyline_block*>& skyline_index_v,
    ...
    skyline_index_u_reverse.resize(g.num_v1 + 1);
    skyline_index_v.resize(g.num_v2 + 1);
    skyline_index_v_reverse.resize(g.num_v2 + 1);
    for (vid_t w = 0; w < g.num_v1 + 1; w++){
        skyline_index_u[w] = new skyline_block;
        skyline_index_u_reverse[w] = new skyline_block;
    }
    for (vid_t w = 0; w < g.num_v2 + 1; w++){
        skyline_index_v[w] = new skyline_block;
        skyline_index_v_reverse[w] = new skyline_block;
    }
    vector<vector<int>> vector_pre_v1;
    vector_pre_v1.resize(g.num_v1);
    vector<vector<int>> vector_pre_v2;
    vector_pre_v2.resize(g.num_v2);
    vector<int> au_vector;
    // build right index
    for (int alpha = 1; alpha < bicore_index_u.size(); alpha++) {
        for (int beta = bicore_index_u[alpha].size() - 1; beta > 0; beta--) {
            if (bicore_index_u[alpha][beta] != NULL) {
                for (auto i = bicore_index_u[alpha][beta]->nodeset.begin(); i != bicore_index_u[alpha][beta]->nodeset.end(); i++) {
                    int m; int n;
                    if (vector_pre_v1[*i].size() != 0){
                        m = vector_pre_v1[*i][0]; n = vector_pre_v1[*i][1];
                    }
                    else{
                        m = -1; n = -1;
                    }
                    ...
                    if (m < alpha && n < beta &&  m != -1 && n != -1){
                        auto pos = skyline_index_u[*i]->mapset.find(m);
                        if (pos != skyline_index_u[*i]->mapset.end()){
                            skyline_index_u[*i]->mapset.erase(pos);
                        }
                    }
                    skyline_index_u_reverse[*i]->mapset[beta] = alpha;
                    au_vector.push_back(alpha);
                    au_vector.push_back(beta);
                    vector_pre_v1[*i] = au_vector;
                    au_vector.clear();
                }
            }
        }
        ...
        for (int alpha = bicore_index_v[beta].size() - 1; alpha > 0; alpha--) {
            if (bicore_index_v[beta][alpha] != NULL) {
                for (auto i = bicore_index_v[beta][alpha]->nodeset.begin(); i != bicore_index_v[beta][alpha]->nodeset.end(); i++) {
                    int m; int n;
                    if (vector_pre_v2[*i].size() != 0){
                        m = vector_pre_v2[*i][0]; n = vector_pre_v2[*i][1];
                    }
                    else{
                        m = -1; n = -1;
                    }
                    ...
                    if (m < beta && n < alpha &&  m != -1 && n != -1){
                        auto pos = skyline_index_v[*i]->mapset.find(m);
                        if (pos != skyline_index_v[*i]->mapset.end()){
                            skyline_index_v[*i]->mapset.erase(pos);
                        }
                    }
                    skyline_index_v_reverse[*i]->mapset[beta] = alpha;
                    au_vector.push_back(beta);
                    au_vector.push_back(alpha);
                    vector_pre_v2[*i] = au_vector;
                    au_vector.clear();
                }
            }
        }
    ...
    for (vid_t u = 0; u < g.num_v1; u++) {
        for(auto it : skyline_index_u_reverse[u]->mapset){
            skyline_index_u[u]->mapset[it.second] = it.first;
        }
    }
    for (vid_t v = 0; v < g.num_v2; v++) {
        for(auto it : skyline_index_v_reverse[v]->mapset){
             skyline_index_v[v]->mapset[it.second] = it.first;
        }
    }
...
void build_bicore_index_space_saver(BiGraph&g, vector<vector<bicore_index_block*>>& bicore_index_u, vector<vector<bicore_index_block*>>& bicore_index_v) {
	bicore_index_u.clear();
	bicore_index_u.resize(g.v1_max_degree + 1);
	bicore_index_v.clear();
	bicore_index_v.resize(g.v2_max_degree + 1);
	// build left
	vector<int> beta_m; beta_m.resize(g.v1_max_degree + 1);
	for (vid_t u = 0; u < g.num_v1; u++) {
		for (int alpha = 1; alpha < g.left_index[u].size(); alpha++) {
...
int binseaech(int* array, int len, int target)
{
	...
	{
		mid = (left + right) / 2;
		if (target == array[2*mid])
		{
			return mid;
		}
		else if (target > array[2*mid])
		{
			left = mid + 1;
		}
		else if (target < array[2*mid])
...
void retrieve_bicore_via_bicore_number (BiGraph& g, vector<skyline_block*>& skyline_index_u, vector<skyline_block*>& skyline_index_v,
	...
	fill_n(left_node.begin(), left_node.size(), false);
	fill_n(right_node.begin(), right_node.size(), false);
	for (int u = 1; u < skyline_index_u.size(); u++){
		skyline_block* block = skyline_index_u[u];
		if (block->mapset.size() == 0) continue;
		if (block->mapset.count(alpha) != 0){
			if (block->mapset[alpha] >= beta){
				left_node[u] = true;
			}
			continue;
		}
		...
		map_test[1] = 2;
		auto iter = block->mapset.end(); 
		iter--;
		if (iter->first < alpha || block->mapset.begin()->second < beta) continue;
		int n = 0;
		for (auto it = block->mapset.begin(); it != block->mapset.end(); it++) {
			if (it->first >= alpha){
				n = it->second;
				break;
			}
		}
		if (n >= beta) left_node[u] = true;
	}
	for (int v = 1; v < skyline_index_v.size(); v++){
...
void retrieve_node_via_index_skyline (BiGraph& g, vector<skyline_block*>& skyline_index_u, vector<skyline_block*>& skyline_index_v, batch_set& pair, vid_t node, bool is_left){
	if(is_left){
		...
		for (auto pair_ij : block->mapset){
			int i = pair_ij.first; int j = pair_ij.second;
			for (int alpha = i_pre + 1; alpha <= i; alpha++){
