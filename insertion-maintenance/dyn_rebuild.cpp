...
namespace Dyn_rebuild
{
	...
	int find_skyline(std::vector<skyline_block*>& skyline_index_uv, vid_t uv, int alpha_or_beta, int is_alpha, int num, std::vector<int>& store_skyline_index_uv){
	    if(skyline_index_uv[uv]->vector_set.size() == 0){
            return 0;
	    }
	    else{
            if(!store_skyline_index_uv[uv]){
                if (is_alpha == 1){
                    if(alpha_or_beta > skyline_index_uv[uv]->vector_set[skyline_index_uv[uv]->vector_set.size() - 2]) return 0;
                    int i = skyline_index_uv[uv]->vector_set.size();
                    while(true){
                        i -= 2;
                        if(skyline_index_uv[uv]->vector_set[i] >= alpha_or_beta && (skyline_index_uv[uv]->vector_set[i - 2] < alpha_or_beta || i - 2 < 0)) break;
                    }
                    store_skyline_index_uv[uv] = i;
                    return skyline_index_uv[uv]->vector_set[i+1];
                }
                else{
                    if(alpha_or_beta > skyline_index_uv[uv]->vector_set[1]) return 0;
                    int i = -1;
                    while(true){
                        i += 2;
                        if(skyline_index_uv[uv]->vector_set[i] >= alpha_or_beta && (skyline_index_uv[uv]->vector_set[i + 2] < alpha_or_beta || i + 2 >= skyline_index_uv[uv]->vector_set.size())) break;
                    }
                    store_skyline_index_uv[uv] = i;
                    return skyline_index_uv[uv]->vector_set[i-1];
                }
                ...
                if (store_skyline_index_uv[uv] > skyline_index_uv[uv]->vector_set.size() - 2) store_skyline_index_uv[uv] = skyline_index_uv[uv]->vector_set.size() - 2;
                else if (store_skyline_index_uv[uv] < 0) store_skyline_index_uv[uv] = 0;
                if (is_alpha == 1){
                    int i = store_skyline_index_uv[uv];
                    while(true){
                        if(i != 0 && i < skyline_index_uv[uv]->vector_set.size() - 2){
                            if(skyline_index_uv[uv]->vector_set[i] == alpha_or_beta){
                                store_skyline_index_uv[uv] = i;
                                return skyline_index_uv[uv]->vector_set[i+1];
                            }
                            else if(skyline_index_uv[uv]->vector_set[i] > alpha_or_beta && skyline_index_uv[uv]->vector_set[i - 2] >= alpha_or_beta) i -= 2;
                            else if(skyline_index_uv[uv]->vector_set[i] < alpha_or_beta && skyline_index_uv[uv]->vector_set[i + 2] < alpha_or_beta) i += 4;
                            else if(skyline_index_uv[uv]->vector_set[i] >= alpha_or_beta && skyline_index_uv[uv]->vector_set[i - 2] < alpha_or_beta){
                                store_skyline_index_uv[uv] = i;
                                return skyline_index_uv[uv]->vector_set[i+1];
                            }
                            else if(skyline_index_uv[uv]->vector_set[i] < alpha_or_beta && skyline_index_uv[uv]->vector_set[i + 2] >= alpha_or_beta){
                                store_skyline_index_uv[uv] = i + 2;
                                return skyline_index_uv[uv]->vector_set[i+3];
                            }
                        }
                        else if(i == 0){
                            if(skyline_index_uv[uv]->vector_set[i] == alpha_or_beta){
                            ...
                            else if(skyline_index_uv[uv]->vector_set[i] > alpha_or_beta){
                                store_skyline_index_uv[uv] = i;
                                return skyline_index_uv[uv]->vector_set[i+1];
                            }
                            else if(skyline_index_uv[uv]->vector_set[i] < alpha_or_beta && skyline_index_uv[uv]->vector_set[i + 2] >= alpha_or_beta){
                        ...
                        else if(i >= skyline_index_uv[uv]->vector_set.size() - 2){
                            if (i == skyline_index_uv[uv]->vector_set.size() - 2){
                                if(skyline_index_uv[uv]->vector_set[i] == alpha_or_beta){
                                    store_skyline_index_uv[uv] = i;
                                    return skyline_index_uv[uv]->vector_set[i+1];
                                }
                    ...
                    int i = store_skyline_index_uv[uv];
                    while(true){
                        if(i > 1 && i != skyline_index_uv[uv]->vector_set.size() - 1){
                            if(skyline_index_uv[uv]->vector_set[i] == alpha_or_beta){
                                store_skyline_index_uv[uv] = i;
                                return skyline_index_uv[uv]->vector_set[i-1];
                            }
                        ...
                        else if(i == skyline_index_uv[uv]->vector_set.size() - 1){
                            if(skyline_index_uv[uv]->vector_set[i] == alpha_or_beta){
                                store_skyline_index_uv[uv] = i;
                                return skyline_index_uv[uv]->vector_set[i-1];
                            }
	...
	double update_skyline_index_swap_with_bfs(BiGraph& g, std::vector<skyline_block*>& skyline_index_u, std::vector<skyline_block*>& skyline_index_v, vid_t u, vid_t v){
		...
		if(beta_2 == 0){
			for(auto node : g.neighbor_v1[u]){
				if(g.right_index[node][1] < g.degree_v1[u]) g.right_index[node][1] = g.degree_v1[u];
			}
			beta_2 = g.left_index[u][g.degree_v1[u]];
		}
        ...
        for(int i_iter = skyline_index_u[u]->vector_set.size()-2; i_iter >= 0; i_iter-=2){
				...
				if (g.left_index[u][current_alpha+1] <= compute_vbeta_regarding_specific_alpha(g,v,current_alpha+1) && skyline_index_u[u]->vector_set[i_iter + 2] != current_alpha + 1){
					...
					if(beta >= skyline_index_u[u]->vector_set[i_iter+3]){
						if (g.left_index[u][current_alpha+1] <= beta) {
							if (g.right_index[v][beta] < current_alpha + 1){
								for (auto nbr_node : nbr_v){
									C.push_back(compute_ualpha_regarding_specific_beta(g,nbr_node,current_beta));
								}
								int alpha_v_ = find_kth(C, C.size(),current_beta);
								if(g.right_index[v][current_beta] < alpha_v_) g.right_index[v][current_beta] = alpha_v_;
							}
					...
					B.clear();
					C.clear();
					if(g.left_index[u][current_alpha + 1] < beta) g.left_index[u][current_alpha + 1] = beta;
					if(g.left_index[u][current_alpha + 1] == beta + 1){
						have_update[F] = 1;
					}
					F.clear();
				}
            ...
            if (D.size() !=0 || current_beta - 1 != 0) alpha_u_whether_alpha_add = find_kth(D, D.size(),current_beta - 1); // ԭ�� alpha_is_u_alpha_add
            else alpha_u_whether_alpha_add = current_beta;
            D.clear();

			if(current_alpha != g.degree_v1[u] -1 && g.left_index[u][current_alpha+1] <= compute_vbeta_regarding_specific_alpha(g,v,current_alpha+1)){
				F.push_back(current_alpha + 1);
		...
		if(alpha_2 == 0){
			for(auto node : g.neighbor_v2[v]){
				if(g.left_index[node][1] < g.degree_v2[v]) g.left_index[node][1] = g.degree_v2[v];
			}
			alpha_2 = g.right_index[v][g.degree_v2[v]];
		}
		...
		for(int i_iter = 1; i_iter <= skyline_index_v[v]->vector_set.size() - 1; i_iter+=2){
			...
			if (current_beta >= g.left_index[u][current_alpha]){
				if (g.right_index[v][current_beta+1] <= compute_ualpha_regarding_specific_beta(g,u,current_beta+1) && skyline_index_v[v]->vector_set[i_iter + 2] != current_beta + 1){
					...
					if(alpha >= skyline_index_v[v]->vector_set[i_iter-3] && have_update.count(F) == 0){
						if (g.right_index[v][current_beta+1] <= alpha) {
							if (g.left_index[u][alpha + 1] < current_beta){
								for (auto nbr_node : nbr_u){
									C.push_back(compute_vbeta_regarding_specific_alpha(g,nbr_node,current_alpha));
								}
								int beta_u_ = find_kth(C, C.size(),current_alpha);
								if(g.left_index[u][current_alpha] < beta_u_) g.left_index[u][current_alpha] = beta_u_;
							}
					...
					B.clear();
					C.clear();
					if(g.right_index[v][current_beta + 1] < alpha) g.right_index[v][current_beta + 1] = alpha;
					if(g.right_index[v][current_beta + 1] == alpha + 1){
						have_update[F] = 1;
					}
					F.clear();
				}
            ...
            if (D.size() !=0 || current_alpha - 1 != 0) beta_v_whether_beta_add = find_kth(D, D.size(),current_alpha - 1);   // ԭ�� beta_is_v_beta_add
            else beta_v_whether_beta_add = current_alpha;
            D.clear();
            // v-beta change
			F.push_back(alpha_3 + 1);
			F.push_back(current_beta + 1);
            if (!g.is_node_change_in_pair_v2[v][current_alpha_and_beta_plus] && have_update.count(F) == 0 && g.right_index[v][current_beta+1] <= compute_ualpha_regarding_specific_beta(g,u,current_beta+1)){
					if(alpha_u >= alpha_3 && compute_ualpha_regarding_specific_beta(g, u, current_beta + 1) >= alpha_3 - 1){
						if (beta_v_whether_beta_add >= alpha_3 + 1 && beta_v_whether_beta_add_ >= alpha_3 + 1){
        ...
        auto end = chrono::system_clock::now();
		rebuild_skyline_index(g, skyline_index_u, skyline_index_v);
		chrono::duration<double> elapsed_seconds = end - start;
		return elapsed_seconds.count();
	}
}